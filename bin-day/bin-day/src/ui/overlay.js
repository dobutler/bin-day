...
import {
  ...
  buySubscription, tipRun, binLabel, dayName,
  ...
  return `
    <header class="hud">
      ...
      <div class="hud-meters">
        <div class="meter"><label>Standing</label><div class="bar"><i id="bar-standing"></i></div></div>
...
function render() {
  ...
  q('#hud-next').innerHTML = today.length
    ? `<b>Collection was this morning:</b> ${today.map((t) => rs.bins[t].name).join(', ')}`
    : next
      ? `Next collection: <b>${dayName(next.day)}</b> (in ${next.day - state.day} day${next.day - state.day === 1 ? '' : 's'}) — ${next.bins.map((t) => rs.bins[t].name).join(', ')}`
  ...
  const contents = Object.entries(state.bins)
    .map(([key, bin]) => {
      ...
      const rows = bin.items
        ...
        .join('');
      return `<details><summary>${binLabel(bin, rs)} (${bin.items.length})</summary><ul>${rows}</ul></details>`;
    })
    .join('');
...
function phoneView() {
  ...
  const favour =
    state.favour && state.favour.status === 'asked'
      ? `<div class="favour">
           <p>${state.favour.from} needs their ${state.favour.binType} bin out by
...
function describeOp(op, rs) {
  const binName = (id) => (rs.bins[id] ? rs.bins[id].name : id);
