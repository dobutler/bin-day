...
import { rulesFor, nameOf, colourFor, isAvailable } from '../systems/state.js';
import { stickerOf } from '../data/palette.js';
...
export class Street extends Phaser.Scene {
  ...
  placeBin(key, homeX, rs) {
    ...
    if (!view) {
      ...
      view = this.binViews[key] = { container, body, label, sticker, number, homeX };
    }
  ...
  paintBin(view, bin, rs) {
    ...
    // Away on somebody else's drive after a muddle.
    const away = !isAvailable(this.state, bin.key || bin.type) && !bin.neighbour;
    view.container.setAlpha(away ? 0.28 : 1);
    if (view.sticker) view.sticker.setAlpha(away ? 0.28 : 1);
    if (view.number) view.number.setAlpha(away ? 0.28 : 1);

    if (bin.tipped) {
      view.container.setAngle(-72);
      view.label.setAngle(72);
    } else {
      view.container.setAngle(0);
      view.label.setAngle(0);
    }
    ...
    // grime builds up over collection cycles and shows on the body
    if (bin.grime > 0.05) {
      g.fillStyle(0x4a3f2f, Math.min(0.55, bin.grime * 0.6));
      g.fillRoundedRect(-22, 4, 44, 30, { tl: 0, tr: 0, bl: 7, br: 7 });
    }

    // lid — tilts open when overfull
    g.fillStyle(Phaser.Display.Color.IntegerToColor(colour).darken(18).color, 1);
    if (over) {
      g.save();
    ...
    // sticker and house number, drawn as text so any glyph works
    const sticker = stickerOf((this.state.scheme[bin.type] || {}).sticker);
    view.sticker.setText(bin.neighbour ? '' : sticker.glyph);
    view.number.setText(
      this.state.labelled && !bin.neighbour ? this.state.houseNumber : ''
    );

    const pct = Math.round(fill * 100);
    const name = bin.neighbour
      ? `Neighbour's ${nameOf(this.state, bin.type).toLowerCase()}`
      : nameOf(this.state, bin.type);
    const note = bin.tipped ? ' — on its side' : over ? ' — lid open' : away ? ' — missing' : '';
    view.label.setText(`${name}\n${pct}%${note}`);
    view.label.setColor(bin.tipped || over ? '#a02020' : '#2f2f2f');
  }
